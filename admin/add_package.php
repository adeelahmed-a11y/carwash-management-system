...
<div class="card">
<div class="card-body">
<h5 class="pb-5">Add Package</h5>
<form method="post" enctype="multipart/form-data">

<div class="form-group row">
  <label class="col-md-2 text-right pt-1">Title</label>
  <div class="col-md-6">
    <input type="text" name="title" placeholder="Enter package title" class="form-control mb-2" required>
  </div>
</div>
...
if(isset($_POST['submit'])){
	...
	$img=$_FILES['img']['name'];
	$sql="insert into service values('','$title','$category','2','$cost','$price','$discount','$duration','$status','$tools','$img','$description')";
	$result=mysqli_query($con,$sql);
	if($result){
		echo '<script>swal("Successfully", "Package added successfully", "success").then(function() { window.location = "package.php";  });</script>';
	}
	else{
